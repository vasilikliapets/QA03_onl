...
class User:
    ...
    def take_book(self, book):
        if book.status == True:
            print(f"{self.name}, книга {book.name_book} вами получена")
            book.status = False

        elif book.status == False:
            print(f"К сожалению Книга {book.name_book} в данный момент недоступна к чтению")
        elif "Reserved" in book.status:
            if self in book.status:
                print(f"{self.name}, зарезервированная книга {book.name_book} вами получена")
                book.status = False
            else:
                print(f"К сожалению книга {book.name_book} зарезервирована кем-то другим")

    def return_book(self, book):
        book.status = True
        print(f"{self.name}, Спасибо что вернули книгу!")
...
book1 = Book("World", "Andry", 100, 1234567890)
book2 = Book("Exit", "Vanya", 300, 987654321)
book3 = Book("Enter", "Maxima", 500, 5678901233)
